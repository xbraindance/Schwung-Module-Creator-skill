...
description: >
  Use this skill whenever the user wants to create, scaffold, troubleshoot or develop a module for Schwung —
...
## Learned Skills & Troubleshooting Tips

### Display Mirroring Refresh
The web display at `http://move.local:7681` mirrors the device's OLED screen. If the display appears **frozen or stale**, trigger a **one-click jog wheel twist** on the device to refresh the mirroring. The jog wheel input will cause a screen update, which syncs to the web display immediately.

**When to use:** Before taking a screenshot, if the display looks stuck or hasn't updated recently — a quick jog twist ensures you're capturing the current state, not stale content.
...
| `raw_midi` | Bypass host velocity/aftertouch transforms; also bypasses internal knob-touch filter |
| `raw_ui` | Module owns Back button — must call `host_return_to_menu()` manually |
| `aftertouch` | Module uses aftertouch |
| `skip_led_clear` | Host skips LED clearing on load/unload — preserves Move's native pad colors |

**Optional module.json fields:**
- `defaults` — pass initial parameters to DSP at load time: `{"defaults": {"preset": 0, "gain": 50}}`
- `ui_chain` — filename of Signal Chain UI shim (see ui_chain.js section below)
- `default_forward_channel` — default MIDI forward channel for shadow slots (-2=passthrough, 1-16=specific)
...
## Signal Chain UI Shim (ui_chain.js)

Modules used as a **MIDI source** in Signal Chain can expose a full-screen UI via `ui_chain.js`. Add `"ui_chain": "ui_chain.js"` to module.json, then export `globalThis.chain_ui`:

```javascript
// ui_chain.js — do NOT override globalThis.init/tick here
import { init, tick, onMidiMessageInternal, onMidiMessageExternal } from './ui_core.mjs';
...
The host ignores `ui_chain.js`; it is loaded by the Signal Chain UI when a module is used as a MIDI source. Ship `ui_chain.js` in your build/install step.
...
// Acceleration: slow turns (>150ms) = step 1, fast turns (<25ms) = step 10
// Hold Shift while turning = always step 1 (fine control)
```
...
- `allow_new_file: true` — show a "+ New File" action in the file browser
- `command` — shell command for non-interactive (headless) tool modules
- `overtake: false` — prevent tool from using overtake display mode
...
const err = host_module_get_error();  // Get error from last DSP operation

// Send MIDI into the loaded DSP module from JS
host_module_send_midi([0x90, 60, 100], "internal");  // source: "internal"|"external"|"host"

// Send MIDI to external USB-A port
move_midi_external_send([0x00, 0x90, 60, 100]);  // [cable, status, d1, d2]

// Send MIDI to Move hardware (LEDs, etc.)
move_midi_internal_send([0x09, 0x90, 68, 127]);  // [type, status, note, value]
// type 0x09 = note (pad LEDs), 0x0b = CC (button LEDs)

// Screen reader (if enabled)
host_announce_screenreader("Loading preset");
```
...
**⚠️ Always ask the user for permission before building and deploying to the device.** Building and deploying modifies the live device and may interrupt the user's work. Confirm intent before running any build or install command.
...
| `text_scroll.mjs` | `createTextScroller`, `getMenuLabelScroller` — auto-scrolling long labels |
| `scrollable_text.mjs` | Scrollable text display component |
| `sampler_overlay.mjs` | Quantized sampler UI |
| `screen_reader.mjs` | Accessibility: `announce`, `announceMenuItem`, `announceView` |
| `sound_generator_ui.mjs` | Shared UI patterns for sound generator modules |
| `chain_param_utils.mjs` | Utilities for Signal Chain parameter handling |
| `chain_ui_views.mjs` | Shared views for Signal Chain UI shims |
...
2. **Before navigating to the display, restore device context:**
   - Remember which **track** was active and which **Schwung slot** held the module being debugged
   - After any restart, use hotkey combos to navigate back to the correct screen:
     - **Shift+Vol+Track 1–4**: Open shadow mode / jump to that slot's settings
     - **Shift+Vol+Menu**: Jump to Master FX settings
     - **Shift+Vol+Step13**: Open Tools menu
     - **Shift+Vol+Jog Click**: Open Overtake menu
   - Navigate into the correct module/menu level before screenshotting
3. **Navigate to the display:** Claude will automatically connect and navigate to `http://move.local:7681`
4. **If the display appears frozen or stale**, trigger a **one-click jog wheel twist** on the device to refresh the screen mirroring before capturing.
5. **Capture the display:** Take the screenshot. If it comes back completely black (no content visible within the grey outline), that means the mirroring hasn't updated — trigger a jog wheel twist and try again. Do not keep retrying without an interaction; if the screen remains black after a jog twist, the device may need a module reload or restart.
6. **Zoom into the grey-outlined box** that frames the 128×64 OLED display — include the full grey border, not just the inner black area. This gives clean, analyzable output.
...
**⚠️ Always confirm with the user before building or deploying.** These steps modify the live device.

#### Option 1: Build Locally (Cross-Compiler)

Requires ARM64 cross-compiler toolchain installed:

```bash
# Navigate to module directory
cd src/your-module

# Build with cross-compiler prefix for ARM64
CROSS_PREFIX=aarch64-unknown-linux-gnu- ./scripts/build.sh

# Output: dist/your-module-module.tar.gz (ready to deploy)
```

**Prerequisites:**
```bash
# Install cross-compiler on macOS (via Homebrew)
brew install aarch64-unknown-linux-gnu-gcc
...
No toolchain installation needed — Docker handles it:

```bash
# Build inside Docker container (ARM64 target)
docker run --rm -v $(pwd):/workspace -w /workspace/src/your-module \
  ubuntu:22.04 bash -c \
...
After building (using either method):

```bash
# Deploy to device at /data/UserData/schwung/
...
# Verify deployment
ssh ableton@move.local 'ls -la /data/UserData/schwung/modules/audio_fx/your-module/'
```
...
# 2. Deploy to device
scp dist/your-module-module.tar.gz ableton@move.local:/data/UserData/schwung/
ssh ableton@move.local 'cd /data/UserData/schwung && tar -xzf your-module-module.tar.gz && rm your-module-module.tar.gz'
...
# 5. View device display to verify
# Open: http://move.local:7681 in Chrome
```
...
# Clear log file (grows unbounded)
...
**Note:** After enabling logging, restart shadow UI (exit and re-enter via Shift+Vol+Track) for logging to take effect.

**JS logging options:**
```javascript
// Option 1: console.log — auto-routed to debug.log
console.log('my-module: something happened');

// Option 2: installConsoleOverride — adds module name prefix automatically
import { installConsoleOverride } from '../../shared/logger.mjs';
installConsoleOverride('my-module');  // call once at startup
console.log('Hello');  // logs as: [my-module] Hello
```

**C DSP logging:**
```c
#include "host/unified_log.h"
LOG_DEBUG("my-module", "Value: %d", value);
LOG_ERROR("my-module", "Failed: %s", reason);
// Levels: LOG_ERROR(0), LOG_WARN(1), LOG_INFO(2), LOG_DEBUG(3)
```
...
Understanding how MIDI flows in shadow mode is critical for debugging:

**Move's USB-MIDI cable numbers:**
- **Cable 0 (In)**: Hardware controls — pads, knobs, buttons (routed to Shadow UI)
- **Cable 2 (Out)**: Track MIDI output from Move sequencer (routed to shadow slots/synths)
- **Cable 0 (Out)**: Move UI events — **filtered out** by shim, never reaches shadow slots

**Key implication:** Shadow synths receive MIDI from Move's **sequencer tracks** (cable 2), not from Move's control surface directly. Configure Move tracks to play on the same MIDI channel as the shadow slot's receive channel.

**Controls intercepted by shadow UI (not passed to Move):**
- CC 14 (jog), CC 3 (jog click), CC 51 (back) — navigation
- CC 71–78 (knobs 1–8) — routed to focused shadow slot's DSP
- Notes 0–9 (knob touches) — filtered

**Controls passed to both shadow UI and Move:**
- CC 40–43 (track buttons) — switches shadow slot AND Move track simultaneously

**Shadow slot config** is persisted at `/data/UserData/schwung/shadow_chain_config.json`.
...
globalThis.tick = function() {
    if (scroller.tick()) redraw();  // scroll position changed
}

// On selection change, reset the scroller
function onSelectionChanged() {
    scroller.setSelected(selectedIndex);
    redraw();
}

// When rendering a label:
const displayText = scroller.getScrolledText(fullLabel, maxChars);
// Scroll behavior: 2s delay, ~100ms steps, 2s pause at end, then reset
```
